...
classdef hybPolyZono < contSet
    ...
    methods
        % Constructor
        function obj = hybPolyZono(varargin)
            if nargin == 8 || nargin == 9 || nargin == 10
                ...
                obj.b  = varargin{7};
                obj.EC = varargin{8};
                % NEW: optional GI, id (keep backward compatibility)
                if nargin >= 9
                    obj.GI = varargin{9};
                else
                    obj.GI = [];
                ...
                % obj.GI = varargin{9};
                % obj.id = varargin{10};
                % [Ac,b,EC,GI,id] = setDefaultValues({A,b,EC,GI,id},varargin(4:end));
                ...
                % NEW: defaults for GI/id
                if isempty(obj.GI); obj.GI = zeros(size(obj.c,1),0); end
                if isempty(obj.id)
                    % align with conPolyZono default
                    obj.id = (1:size(obj.E,1))';
                end
            elseif nargin == 1
        ...
        [v,f] = plotAsConPolyZono(obj,optSolver);   % Plot as the union of constrained zonotopes
        % [v,f] = plotHybZono1D(obj,optSolver);   % Plot in 1 dimension
        [v,f] = plotHybPolyZono2D(obj,optSolver);   % Plot in 2 dimensions
        % [v,f] = plotHybZono3D(obj,optSolver);   % Plot in 3 dimensions
        [leaves] = getPolyLeaves(obj,optSolver)     % Identify non-empty combinations of binary factors
        
         % Export each leaf as a cell array of conZonos 
        function out = hyb2leafArray(obj)
            leaves = obj.getPolyLeaves();
            for leaf = 1: length(leaves)
                % out{leaf,1} = conZono(obj.Gc,obj.c+obj.Gb*leaves(:,leaf),obj.Ac,obj.b-obj.Ab*leaves(:,leaf));
                out{leaf,1} = conPolyZono(obj.c+obj.Gb*leaves(:,leaf),obj.Gc,obj.E,obj.Ac,obj.b-obj.Ab*leaves(:,leaf),obj.EC);
            end
        end
    end
end